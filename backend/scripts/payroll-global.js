...
// Seeds payroll run for Lina, Eric, and Charlie
...
async function main() {
  ...
  try {
    ...
    // ============================================
    // 1. CREATE/FIND PAYROLL TEAM MEMBERS
    // ============================================
    console.log('👥 Creating/Finding Payroll Team Members...\n');

    // === SARAH SMITH - PAYROLL SPECIALIST ===
    let sarah = await db.collection('employee_profiles').findOne({
      workEmail: 'sarah.smith@company.com',
    });

    if (!sarah) {
      console.log('   ⚠️  Sarah (Payroll Specialist) not found, creating...');
      const sarahResult = await db.collection('employee_profiles').insertOne({
        employeeNumber: 'EMP-SARAH',
        ...
        mobilePhone: '+201012345001',
        dateOfBirth: new Date(1990, 0, 1),
        gender: 'FEMALE',
        maritalStatus: 'SINGLE',
        dateOfHire: new Date('2019-01-01'),
        ...
        biography: 'Payroll Specialist - Sarah Smith',
        role: 'PAYROLL_SPECIALIST',
        createdAt: now,
        updatedAt: now,
      });
      sarah = { _id: sarahResult.insertedId };
      ...
      console.log(`   ✅ Created Sarah (Payroll Specialist): ${sarah._id}`);
    } else {
      console.log(`   ✅ Found Sarah (Payroll Specialist): ${sarah._id}`);
    }

    // === MICHAEL JOHNSON - PAYROLL MANAGER ===
    let michael = await db.collection('employee_profiles').findOne({
      workEmail: 'michael.johnson@company.com',
    });

    if (!michael) {
      console.log('   ⚠️  Michael (Payroll Manager) not found, creating...');
      const michaelResult = await db.collection('employee_profiles').insertOne({
        employeeNumber: 'EMP-MICHAEL',
        ...
        mobilePhone: '+201012345002',
        dateOfBirth: new Date(1988, 0, 1),
        gender: 'MALE',
        maritalStatus: 'MARRIED',
        dateOfHire: new Date('2017-01-01'),
        ...
        biography: 'Payroll Manager - Michael Johnson',
        role: 'PAYROLL_MANAGER',
        createdAt: now,
        updatedAt: now,
      });
      michael = { _id: michaelResult.insertedId };

      // Create user account for Michael
      await db.collection('users').insertOne({
        email: 'michael.johnson@company.com',
        password: hashedPassword,
        role: 'PAYROLL_MANAGER',
        employeeId: michael._id,
        isActive: true,
        createdAt: now,
        updatedAt: now,
      });

      console.log(`   ✅ Created Michael (Payroll Manager): ${michael._id}`);
    } else {
      console.log(`   ✅ Found Michael (Payroll Manager): ${michael._id}`);
    }

    // === EMILY WILLIAMS - FINANCE STAFF ===
    let emily = await db.collection('employee_profiles').findOne({
      workEmail: 'emily.williams@company.com',
    });

    if (!emily) {
      console.log('   ⚠️  Emily (Finance Staff) not found, creating...');
      const emilyResult = await db.collection('employee_profiles').insertOne({
        employeeNumber: 'EMP-EMILY',
        ...
        biography: 'Finance Staff - Emily Williams',
        role: 'FINANCE_STAFF',
        createdAt: now,
        updatedAt: now,
      });
      emily = { _id: emilyResult.insertedId };

      // Create user account for Emily
      await db.collection('users').insertOne({
        email: 'emily.williams@company.com',
        password: hashedPassword,
        role: 'FINANCE_STAFF',
      ...
      console.log(`   ✅ Created Emily (Finance Staff): ${emily._id}`);
    } else {
      console.log(`   ✅ Found Emily (Finance Staff): ${emily._id}`);
    }

    // ============================================
    // 2. CREATE/FIND REGULAR EMPLOYEES
    // ============================================
    console.log('\n👥 Creating/Finding Regular Employees...\n');
    ...
    console.log(`   • Specialist: Sarah Smith (${sarah._id})`);
