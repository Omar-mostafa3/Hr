...
export class PayrollExecutionController {
  ...
  @Post('payroll-runs/:runId/generate-draft')
@Roles(SystemRole.PAYROLL_SPECIALIST)
async generatePayrollDraft(@Param('runId') runId: string) {
    return await this.payrollExecutionService.generatePayrollDraft(runId);
}
