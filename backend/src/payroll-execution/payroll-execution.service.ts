...
import { BonusStatus, BenefitStatus, PayRollStatus, PaySlipPaymentStatus, BankStatus, PayRollPaymentStatus } from './enums/payroll-execution-enum';
...
export class PayrollExecutionService {
  ...
  constructor(
    ...
    @InjectModel(Department.name)  // ✅ ADD THIS LINE
  ...
  async getAllPayslips(
    ...
    return payslips.map((payslip) => {
      ...
      return {
        ...
        runId: run?.runId || run?._id?.toString(),  // ✅ ADD THIS LINE
  ...
  async approveBenefit(id: string) {
    ...
    // ✅ If terminationId is missing, find and set it
    if (!benefit.terminationId) {
      // Find the termination record for this employee
      const termination = await this.terminationAndResignationBenefitsModel
        .findOne({
      ...
      if (termination) {
        benefit.terminationId = termination._id;
      } else {
        // If no termination found, create a placeholder or throw error
        throw new BadRequestException(
          `No termination record found for employee ${benefit.employeeId}`
        );
      }
    }
...
async rejectBenefit(id: string) {
  ...
  async startPayrollInitiation(
    ...
    try {
      console.log('🚀 Starting payroll initiation...', { 
        runId, 
        payrollPeriod, 
        entity,
        payrollSpecialistId 
      ...
      // ✅ Step 1: Check pre-run approvals
      console.log('📋 Checking pre-run approvals...');
      const preRunCheck = await this.checkPreRunApprovalsComplete();
      console.log('✅ Pre-run check result:', JSON.stringify(preRunCheck, null, 2));
      
      if (!preRunCheck.allApprovalsComplete) {
        throw new BadRequestException({
          message: 'Cannot start payroll. Pending pre-run approvals must be completed first.',
          details: preRunCheck,
        });
      }
  
      // ✅ Step 2: Validate payroll period
      console.log('📅 Validating payroll period...');
      const validation = await this.validatePayrollPeriod(payrollPeriod);
      console.log('✅ Period validation result:', JSON.stringify(validation, null, 2));
      ...
      // ✅ Step 3: Count active employees WITH DETAILED LOGGING
      console.log('👥 Fetching active employees...');
      console.log('📊 Using status filter:', EmployeeStatus.ACTIVE);
      ...
      // Then check active employees
      const activeEmployeesCount = await this.employeeModel
        .countDocuments({ status: EmployeeStatus.ACTIVE })
        .exec();
      
      console.log(`✅ Active employees found: ${activeEmployeesCount} out of ${totalEmployees} total`);
  
      // ✅ IMPORTANT: Check if we have employees
      if (totalEmployees === 0) {
        throw new BadRequestException({
          message: 'No employees found in the system',
          hint: 'Please add employees before creating a payroll run'
        });
      }
  
      if (activeEmployeesCount === 0) {
        // Get status breakdown for debugging
        const statusBreakdown = await this.employeeModel.aggregate([
          { $group: { _id: '$status', count: { $sum: 1 } } }
        ]).exec();
        
        console.log('⚠️ No active employees. Status breakdown:', statusBreakdown);
        
        throw new BadRequestException({
          message: 'No active employees found to process payroll',
          hint: 'Check employee status values. Expected status: "' + EmployeeStatus.ACTIVE + '"',
          statusBreakdown: statusBreakdown
        });
      }
  
      // ✅ Step 4: Check for existing run with same runId
      console.log('🔍 Checking for duplicate runId...');
      const existingRun = await this.payrollRunsModel.findOne({ runId }).exec();
      if (existingRun) {
        throw new BadRequestException({
          message: `Payroll run with ID "${runId}" already exists`,
      ...
      const payrollRun = new this.payrollRunsModel({
        ...
        entity,
        employees: activeEmployeesCount, // ✅ Store actual count
        exceptions: 0,
        totalnetpay: 0,
        payrollSpecialistId,
      ...
      return {
        ...
        payrollRun: {
          ...
          employees: payrollRun.employees,
          payrollSpecialistId: payrollRun.payrollSpecialistId,
          // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
          // createdAt: payrollRun.createdAt,
        },
        summary: {
          totalEmployees: activeEmployeesCount,
          pendingSigningBonuses: preRunCheck.pendingSigningBonuses,
          pendingBenefits: preRunCheck.pendingBenefits,
        },
        nextStep: 'Call generatePayrollDraft() to process employee salaries and generate draft',
      };
  ...
  async generatePayrollDraft(runId: string) {
    try {
      const runObjectId = await this._resolveRunObjectId(runId);
      ...
      const existingDetails = await this.employeePayrollDetailsModel.countDocuments({
        payrollRunId: runObjectId,
      });
  
      if (existingDetails > 0) {
        return {
          message: 'Draft already exists for this payroll run',
          payrollRun: run,
          summary: {
            totalEmployees: run.employees,
            exceptionsCount: run.exceptions,
            totalNetPay: run.totalnetpay,
          },
          alreadyExists: true,
          nextStep: 'Review draft and publish for approval',
        };
      }
  
      if (run.status !== PayRollStatus.DRAFT) {
        throw new BadRequestException(
          `Cannot generate draft for payroll run with status: ${run.status}. Run must be in DRAFT status.`
        );
      }
      ...
      const updatedRun = await this.payrollRunsModel.findById(runObjectId).exec();
      
      return {
        message: 'Payroll draft generated successfully',
        payrollRun: updatedRun,
        summary: {
          totalEmployees: result.run.employees,
      ...
      throw new BadRequestException({
        message: 'Failed to generate payroll draft',
        error: error.message,
        stack: process.env.NODE_ENV === 'development' ? error.stack : undefined
      });
    }
  ...
  async regeneratePayrollDraft(runId: string, force: boolean = false) {
    try {
      console.log('🔄 Regenerating draft for runId:', runId);
      ...
      if (run.status !== PayRollStatus.DRAFT && !force) {
        throw new BadRequestException(
          `Cannot regenerate draft for payroll run with status: ${run.status}. Use force=true to override.`
        );
      }
  
      // Delete existing draft
      const deleteResult = await this.employeePayrollDetailsModel.deleteMany({
        payrollRunId: runObjectId,
      });
  
      console.log(`🗑️ Deleted ${deleteResult.deletedCount} existing employee records`);
      ...
      // Generate new draft
      return await this.generatePayrollDraft(runId);
      
    } catch (error) {
      console.error('❌ Error in regeneratePayrollDraft:', error);
      throw error;
    }
  }
  ...
  async freezePayroll(runId: string, reason?: string) {
    try {
      ...
      if (existingPayslips === 0) {
        ...
        payslipsGenerated = await this.generatePayslips(runId);
      ...
      // ✅ ADD THIS: Update all payslips to PAID status
      const updateResult = await this.payslipModel.updateMany(
        { payrollRunId: runObjectId },
        { 
          $set: { 
            paymentStatus: 'PAID',
            paidAt: new Date()  // Optional: track when it was marked as paid
          } 
        }
      );
      console.log('💰 Updated payslips to PAID:', updateResult.modifiedCount);
      ...
      return { 
        message: 'Payroll run locked and marked as paid', 
        runId: run.runId,
        payslipsCount: existingPayslips || payslipsGenerated?.count,
        payslipsUpdatedToPaid: updateResult.modifiedCount
      };
  ...
  async createPayrollAdjustment(
    ...
    // FIX: Check if bankStatus needs normalization (stored as wrong string in DB)
    if ((details.bankStatus as any) === 'VALID') {
      details.bankStatus = BankStatus.VALID;
    } else if ((details.bankStatus as any) === 'MISSING') {
      details.bankStatus = BankStatus.MISSING;
    }
    ...
    try {
      ...
      for (const d of detailsList) {
        try {
          ...
          console.error(
            '❌ Error creating payslip for employee:',
            d.employeeId,
          );
  ...
  async distributePayslips(runId: string) {
    ...
    const res = await this.payslipModel.updateMany(
          { payrollRunId: runObjectId },
          { $set: { paymentStatus: PaySlipPaymentStatus.PAID as any } }
        );
    return {
  ...
  async reviewPayrollDraft(payrollRunId: string) {
    ...
    // ✅ Use aggregation to properly join all collections
    const details = await this.employeePayrollDetailsModel.aggregate([
      { $match: { payrollRunId: runObjectId } },
      {
        $lookup: {
      ...
      { $unwind: { path: '$employeeData', preserveNullAndEmptyArrays: true } },
      {
        $lookup: {
          from: 'departments',
          localField: 'employeeData.primaryDepartmentId',
          foreignField: '_id',
          as: 'departmentData'
        }
      },
