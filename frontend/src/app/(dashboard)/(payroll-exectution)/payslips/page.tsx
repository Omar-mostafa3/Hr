...
import { Search, Download, Send, Eye, Printer, Mail, X, FileText, AlertTriangle, Loader2, CheckCircle } from 'lucide-react';
...
const FinalizedPayslipsPage = () => {
  ...
  const [filters, setFilters] = useState({ runId: '', employeeName: '', department: '' });
  ...
  const handleSelectPayslip = (id: string) => {
    setSelectedPayslips(prev => prev.includes(id) ? prev.filter(p => p !== id) : [...prev, id]);
  };
  ...
  const handleDownload = async (payslipId: string) => {
    try {
      ...
      const response = await fetch(`${API_URL}/payroll-execution/payslips/${payslipId}/download`, { method: 'POST' });
      if (!response.ok) throw new Error('PDF generation endpoint not available');
  ...
  const handleBulkDownload = async () => {
    if (selectedPayslips.length === 0) { alert('Please select payslips first'); return; }
    try {
      setActionLoading(true);
      ...
      if (!response.ok) throw new Error('Bulk download endpoint not available');
      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
  ...
  const handleBulkResend = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Bulk resend endpoint not available');
  ...
  const handleDistributeByRunId = async (runId: string) => {
    if (!runId) { alert('No Run ID found for this payslip'); return; }
    if (!confirm('Distribute all payslips for this payroll run? This will send payslips to employees.')) return;
    try {
      ...
      const result = await response.json();
      alert(`✅ Successfully distributed ${result.totalDistributed} payslips to employees!`);
      fetchPayslips();
    } catch (err: any) {
      console.error('Error distributing payslips:', err);
      alert(`Failed to distribute payslips: ${err.message}`);
    } finally {
      setDistributeLoading(false);
    }
  };

  const handleDistributeAllPaid = async () => {
    const paidPayslips = payslips.filter(p => p.status?.toUpperCase() === 'PAID');
    if (paidPayslips.length === 0) { alert('No paid payslips to distribute'); return; }
    const uniqueRunIds = [...new Set(paidPayslips.map(p => p.runId).filter(Boolean))];
    if (uniqueRunIds.length === 0) { alert('No payroll run IDs found for paid payslips'); return; }
    if (!confirm(`Distribute payslips for ${uniqueRunIds.length} payroll run(s)? This will send to employees.`)) return;
    try {
      setDistributeLoading(true);
      ...
      for (const runId of uniqueRunIds) {
        try {
          ...
          console.error(`Failed to distribute for runId ${runId}:`, err);
        }
      }
      alert(`✅ Successfully distributed ${totalDistributed} payslips!`);
      fetchPayslips();
    } catch (err: any) {
      console.error('Error distributing payslips:', err);
      alert(`Failed to distribute payslips: ${err.message}`);
  ...
  const getStatusBadge = (status: string) => {
    ...
    return statusMap[status] || { class: 'bg-gray-200 text-gray-800', label: status };
  };
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-2">
            {paidCount > 0 && (
              <button
                onClick={handleDistributeAllPaid}
              ...
              >
                {distributeLoading ? <Loader2 size={18} className="animate-spin" /> : <Send size={18} />}
                Distribute All Paid ({paidCount})
              </button>
            )}
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="mt-4 flex gap-2 justify-end">
            <button
              onClick={() => { setFilters({ runId: '', employeeName: '', department: '' }); setTimeout(fetchPayslips, 100); }}
              className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition"
            >
              Clear Filters
            </button>
            <button onClick={fetchPayslips} className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition">
              Apply Filters
            </button>
          </div>
        </div>
              ...
              <table className="w-full">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {payslips.map((payslip) => {
                    ...
                    const isPaid = isPaidStatus(payslip.status);
                    return (
                      <tr key={payslip._id} className="hover:bg-gray-50 transition">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex gap-2">
                            ...
                            {isPaid && payslip.runId && (
                              <button
                                onClick={() => handleDistributeByRunId(payslip.runId)}
                                className="text-orange-600 hover:text-orange-800 p-1 hover:bg-orange-50 rounded transition"
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b px-6 py-4 flex justify-between items-center">
              ...
              <button
                onClick={() => { setShowDetailModal(false); setSelectedPayslip(null); }}
              ...
              <div className="p-6 space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between py-2 border-b">
                      ...
                      <span className="font-semibold">${(selectedPayslip.earningsDetails?.baseSalary || 0).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between py-2 border-b">
                      <span className="text-gray-600">Allowances</span>
                      <span className="font-semibold">${(selectedPayslip.earnings?.allowances || 0).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between py-2 border-b">
                      <span className="text-gray-600">Bonuses</span>
                      <span className="font-semibold">${(selectedPayslip.earnings?.bonuses || 0).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between py-2 border-b">
                      <span className="text-gray-600">Benefits</span>
                      <span className="font-semibold">${(selectedPayslip.earnings?.benefits || 0).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between py-3 font-bold text-lg bg-green-50 px-2 rounded">
                      <span>Total Gross</span>
                      <span className="text-green-600">${(selectedPayslip.totalGrossSalary || selectedPayslip.grossSalary || 0).toLocaleString()}</span>
                    </div>
                  </div>
                </div>
                <div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between py-2 border-b">
                      ...
                      <span className="font-semibold text-red-600">-${(selectedPayslip.deductionsBreakdown?.taxes || 0).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between py-2 border-b">
                      <span className="text-gray-600">Insurance</span>
                      <span className="font-semibold text-red-600">-${(selectedPayslip.deductionsBreakdown?.insurance || 0).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between py-2 border-b">
                      <span className="text-gray-600">Penalties</span>
                      <span className="font-semibold text-red-600">-${(selectedPayslip.deductionsBreakdown?.penalties || 0).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between py-3 font-bold text-lg bg-red-50 px-2 rounded">
                      <span>Total Deductions</span>
                      <span className="text-red-600">-${(selectedPayslip.totaDeductions || selectedPayslip.deductions || 0).toLocaleString()}</span>
                    </div>
                  </div>
                </div>
                <div className="bg-blue-50 p-6 rounded-lg border-2 border-blue-200">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-3xl font-bold text-blue-600">${(selectedPayslip.netPay || 0).toLocaleString()}</span>
                  </div>
                </div>
