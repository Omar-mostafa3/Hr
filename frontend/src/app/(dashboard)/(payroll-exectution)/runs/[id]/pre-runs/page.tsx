...
const payrollService = {
  ...
  editBenefit: async (id: string, givenAmount: number, terminationId: string) => {
    const response = await fetch(`http://localhost:3000/payroll-execution/benefits/${id}/edit`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ givenAmount, terminationId })
    });
    if (!response.ok) throw new Error('Failed to edit');
    return response.json();
  }
  ...
  const handleSaveEdit = async (updatedData: any) => {
    try {
      if (editType === 'bonus') {
        ...
        await payrollService.editBenefit(
          editingItem!._id,
          updatedData.givenAmount,
          (editingItem as Benefit).terminationId
        );
      }
        ...
        <tbody className="divide-y divide-gray-200 bg-white">
          {benefits.map((benefit: Benefit) => (
            <tr key={benefit._id} className="hover:bg-gray-50 transition">
              ...
              <td className="px-6 py-4">
                <div className="text-sm font-medium text-gray-900">
                  {benefit.employeeId?.firstName} {benefit.terminationID}
                </div>
