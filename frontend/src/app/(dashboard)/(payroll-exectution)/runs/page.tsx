...
const AllRunsPage = () => {
  ...
  const getQuickActions = (run) => {
    ...
    switch (status) {
      ...
      if (isPayrollSpecialist) {
        actions.push(
          { label: 'Start Payroll', icon: PlayCircle, color: 'green', action: async () => { const {data} = await axios.post(`${API_URL}/payroll-execution/payroll-runs/${run._id}/generate-draft`); await new Promise(resolve => setTimeout(resolve, 1500));  data?.payrollRun && router.push(`/runs/${data.payrollRun._id}/draft`); }},
      ...
      case 'UNDER_REVIEW':
        if (isPayrollSpecialist) {
          actions.push(
            { label: 'Review Draft', icon: FileText, color: 'amber', action: () => router.push(`/runs/${run._id}/draft`) },
            { label: 'Exceptions', icon: AlertTriangle, color: 'orange', action: () => router.push(`/runs/${run._id}/draft`) }
          );
        }
        actions.push(viewAction);
        break;
      ...
      case 'LOCKED':
        ...
        case 'REJECTED':
          if (isPayrollSpecialist) {
            actions.push(
              { label: 'Regenerate Draft', icon: PlayCircle, color: 'green', action: async () => { const {data} = await axios.post(`${API_URL}/payroll-execution/payroll-runs/${run._id}/generate-draft`); data?.payrollRun && router.push(`/runs/${data.payrollRun._id}/draft`); }},
              { label: 'Edit Period', icon: Edit, color: 'slate', action: () => handleEditRun(run) },
              { label: 'View Draft', icon: Eye, color: 'blue', action: () => router.push(`/runs/${run._id}/draft`) }
            );
          } else {
            actions.push(viewAction);
          }
